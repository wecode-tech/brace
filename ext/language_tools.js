ace.define("ace/snippets",["require","exports","module","ace/lib/oop","ace/lib/event_emitter","ace/lib/lang","ace/range","ace/anchor","ace/keyboard/hash_handler","ace/tokenizer","ace/lib/dom","ace/editor"], function(acequire, exports, module) {
    ...
    //                }
            ...
            try {
                ...
                var reg2 = new RegExp(needle,'g');
                for(var i = 0;i < results.length;i++){
                    let t1 = reg1.test(results[i].value)
                    let t2 = reg2.test(results[i].value)
                    t1?a.push(results[i]):t2?b.push(results[i]):c.push(results[i])    
                }
                all = a.concat(b,c)
                return all;
            }catch (err){
    ...
    ace.define("ace/autocomplete/text_completer",["require","exports","module","ace/range"], function(acequire, exports, module) {
        ...
        function getWordIndex(doc, pos) {
            ...
            /*******************/
            /*
            var formattedTextBefore = textBefore
                .replace(/#.*?\n/g, '\n')
                .replace(/\\"|\\'/g, '||')
                .replace(/"[^"]*?"/g, '')
                .replace(/'[^']*?'/g, '');
            */ 
            /*******************/
    //        return formattedTextBefore.split(splitRegex).length - 1;
             return textBefore.split(splitRegex).length - 1;
        }
        function wordDistance(doc, pos) {
            //修改
            /*******************/
            var formattedDocValue = doc.getValue()
                .replace(/#.*?\n/g, '\n')
                .replace(/\\"|\\'/g, '||')
                .replace(/"[^"]*?"/g, '')
                .replace(/'[^']*?'/g, '');
            ...
            var words = formattedDocValue.split(splitRegex);
            ...
            var builtinConstants = (
                "True|False|None|NotImplemented|Ellipsis|__debug__|Turtle|Screen"
            );
        
            // var builtinFunctions = (
            //     "abs|divmod|int|input|intern|open|staticmethod|all|enumerate|ord|str|any|" +
            ...
            words.forEach(function(word, idx) {
                 //修改
                /*******************/
                if (!word || (word === currentWord && currentWord.length > 1)) return;
                //if (!word || word === currentWord) return;
    
                if (finalKeywords.indexOf(word) >= 0) {
                    return;
                }
    
                if (word === currentWord && wordCounts[word] == 1) {
                    return;
                }
    
                if (!/^[a-zA-Z_$][a-zA-Z_$0-9]*$/.test(word)) {
                    return;
                }
                 /*******************/
        ...
        exports.getCompletions = function(editor, session, pos, prefix, callback) {
            ...
            callback(null, wordList.map(function(word) {
                return {
                    ...
                    score: wordList.indexOf(word) + 10000,
                    // meta: "local"
                    meta: "变量"
                };
            }));
